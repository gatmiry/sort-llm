"""
Per-checkpoint analysis for all configs in frequent-checkpoint.
Discovers all LN0/LN1 config pairs, runs analysis workers across 8 GPUs,
and generates comparison plots for every intermediate checkpoint.
"""
import os
import re
import subprocess
...
def discover_config_pairs():
    pat = re.compile(r'^(V\d+_B\d+_LR[\de.+-]+_MI\d+)_LN([01])_' + ARCH_SUFFIX + '$')
    configs = {}
    for d in sorted(os.listdir(BASE_DIR)):
        ...
        if base not in configs:
            configs[base] = {}
        configs[base][ln] = full
    return {k: v for k, v in configs.items() if 0 in v and 1 in v}


def find_iterations(ln0_dir, ln1_dir):
    pat = re.compile(r'_itr(\d+)\.pt$')
    def get_iters(d):
        return {int(pat.search(f).group(1)) for f in os.listdir(d)
                if f.endswith('.pt') and pat.search(f)}
    return sorted(get_iters(ln0_dir) & get_iters(ln1_dir))
...
def build_all_tasks(config_pairs):
    tasks = []
    for config_base, ln_dirs in config_pairs.items():
        iterations = find_iterations(ln_dirs[0], ln_dirs[1])
        for itr in iterations:
            tmp_dir = os.path.join(OUTPUT_DIR, 'tmp_analysis', f'{config_base}_itr{itr}')
            for ln_label, ln_val in [('LN0', 0), ('LN1', 1)]:
                ckpt = ckpt_path(ln_dirs[ln_val], itr)
                for layer in [0, 1]:
                    tasks.append({
                        'config': config_base, 'itr': itr, 'ckpt': ckpt, 'ln': ln_label,
                        'task': 'cinclogits', 'layer': layer,
                        'out': os.path.join(tmp_dir, f'cinclogits_{ln_label}_layer{layer}.npz'),
                        'name': f'{config_base}_itr{itr}_cinclogits_{ln_label}_L{layer}',
                        'extra_args': [],
                    })
                    tasks.append({
                        'config': config_base, 'itr': itr, 'ckpt': ckpt, 'ln': ln_label,
                        ...
                        'name': f'{config_base}_itr{itr}_intensity_{ln_label}_L{layer}',
                        'extra_args': [],
                    })
                    tasks.append({
                        'config': config_base, 'itr': itr, 'ckpt': ckpt, 'ln': ln_label,
                        ...
                        'name': f'{config_base}_itr{itr}_intensity_ub60_{ln_label}_L{layer}',
                    ...
                    tasks.append({
                        ...
                        'name': f'{config_base}_itr{itr}_ablation_{ln_label}_L{layer}',
                        'extra_args': [],
                    })
                tasks.append({
                    'config': config_base, 'itr': itr, 'ckpt': ckpt, 'ln': ln_label,
                    ...
                    'name': f'{config_base}_itr{itr}_baseline_{ln_label}',
                    'extra_args': [],
                })
    return tasks
...
def launch_worker(task, gpu_id):
    ...
    log_dir = os.path.join(OUTPUT_DIR, 'analysis_logs')
...
def run_workers(tasks_to_run):
    ...
    while queue or any(gpu_jobs[g] for g in range(NUM_GPUS)):
        ...
        now = time.time()
        if now - last_print >= 15 or not queue:
            elapsed = now - t0
...
def _plot_ablation(data, plot_dir, tag):
    ...
    if 'cond_acc' not in data[('LN0', 0)] or data[('LN0', 0)]['cond_acc'] is None:
        return
    fig, axes = plt.subplots(1, 2, figsize=(10, 4), sharey=True)
    for i, layer in enumerate([0, 1]):
        ax = axes[i]
...
def assemble_plots(config_pairs):
    for config_base, ln_dirs in sorted(config_pairs.items()):
        iterations = find_iterations(ln_dirs[0], ln_dirs[1])
        plot_parent = os.path.join(OUTPUT_DIR, f'plots_{config_base}_{ARCH_SUFFIX}')
        ...
        for itr in iterations:
            ...
            tmp_dir = os.path.join(OUTPUT_DIR, 'tmp_analysis', f'{config_base}_itr{itr}')
            plot_dir = os.path.join(plot_parent, f'plots_{config_base}_{ARCH_SUFFIX}_ckpt{itr}')
            ...
            try:
                ...
                if itr % 5000 == 0 or itr == iterations[-1]:
                    print(f'  {config_base} ckpt{itr}: {n_plots} plots')
            except Exception as e:
                print(f'  ERROR {config_base} ckpt{itr}: {e}')


def main():
    config_pairs = discover_config_pairs()
    if not config_pairs:
        print('No config pairs found! Check that checkpoint directories exist.')
        return

    print(f'Discovered {len(config_pairs)} config pairs:')
    all_tasks = []
    for config_base, ln_dirs in sorted(config_pairs.items()):
        iters = find_iterations(ln_dirs[0], ln_dirs[1])
        print(f'  {config_base}: {len(iters)} checkpoints ({iters[0]}..{iters[-1]})')
        all_tasks_for_config = build_all_tasks({config_base: ln_dirs})
        all_tasks.extend(all_tasks_for_config)

    tasks_to_run = [t for t in all_tasks if not os.path.exists(t['out'])]
    print(f'\nTotal tasks: {len(all_tasks)}, cached: {len(all_tasks) - len(tasks_to_run)}, '
          f'to run: {len(tasks_to_run)}')

    if tasks_to_run:
        print(f'\nDispatching {len(tasks_to_run)} workers across {NUM_GPUS} GPUs '
              f'(max {MAX_WORKERS_PER_GPU} per GPU)...')
        run_workers(tasks_to_run)

    print(f'\nAssembling plots...')
    assemble_plots(config_pairs)

    total_plots = 0
    for config_base in sorted(config_pairs.keys()):
        plot_parent = os.path.join(OUTPUT_DIR, f'plots_{config_base}_{ARCH_SUFFIX}')
        if os.path.isdir(plot_parent):
            for sub in os.listdir(plot_parent):
                sub_path = os.path.join(plot_parent, sub)
                if os.path.isdir(sub_path):
                    total_plots += len([f for f in os.listdir(sub_path) if f.endswith('.png')])
    print(f'\nDone! Total plots generated: {total_plots}')
