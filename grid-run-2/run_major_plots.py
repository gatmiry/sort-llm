"""
Major plot generation for a set of configs.
Generates the full suite of plots (matching grid-run format) for each
(V, B, LR, MI) config that has both LN0 and LN1 final checkpoints.

Checkpoints are read from CKPT_BASE (grid-run/outputs by default).
Outputs go to OUTPUT_BASE (grid-run-2/outputs by default).

Usage:
  python run_major_plots.py V512_B16_LR1e-02
  python run_major_plots.py V512_B16_LR1e-02 --mi 40000 60000
  python run_major_plots.py V512_B16_LR1e-02 --dry-run
"""
import argparse
import itertools
import os
...
CKPT_BASE = os.path.join(SCRIPT_DIR, '..', 'grid-run', 'outputs')
OUTPUT_BASE = os.path.join(SCRIPT_DIR, 'outputs')
WORKER_SCRIPT = os.path.join(SCRIPT_DIR, 'analyze_worker.py')

ALL_MI = [10000, 20000, 40000, 60000]
UB_VALUES = [10, 15, 20, 30, 50, 60]
...
def find_pairs(config_prefix, mi_list):
    m = re.match(r'V(\d+)_B(\d+)_LR([\d.e+-]+)', config_prefix)
    vs, bs, lr = int(m.group(1)), int(m.group(2)), m.group(3)
    pairs = []
    for mi in mi_list:
        dir_0 = os.path.join(CKPT_BASE, f"V{vs}_B{bs}_LR{lr}_MI{mi}_LN0_E64_H1_L2")
        dir_1 = os.path.join(CKPT_BASE, f"V{vs}_B{bs}_LR{lr}_MI{mi}_LN1_E64_H1_L2")
        ...
        if ckpt_0 and ckpt_1:
            ...
            if not ckpt_1:
                missing.append('LN1')
            print(f"  SKIP MI={mi}: missing {', '.join(missing)}")
    return pairs


def make_all_tasks(pairs):
    tasks = []
    for pair in pairs:
                ...
                if ret is not None:
                    ...
                    if ret == 0:
                        ...
                        if completed % 10 == 0 or completed == total:
                            ...
                            print(f"[PROGRESS] {completed}/{total} done, {failed} fail, {len(queue)} queued ({elapsed:.0f}s)")
                    else:
                        failed += 1
...
# ── Plot assembly (same as grid-run/run_all_analysis.py) ─────────────────────

def _load(tmp_dir, task_type, ln, layer, ub=None):
    if ub is not None:
        f = os.path.join(tmp_dir, f"intensity_{ln}_layer{layer}_ub{ub}.npz")
    else:
        f = os.path.join(tmp_dir, f"{task_type}_{ln}_layer{layer}.npz")
    return np.load(f)


def assemble_all_plots(pair):
    vs, bs, lr, mi = pair['vocab_size'], pair['block_size'], pair['lr'], pair['max_iters']
    base = f"V{vs}_B{bs}_LR{lr}_MI{mi}"
    tmp_dir = os.path.join(OUTPUT_BASE, 'tmp_results', base)

    itr_0 = int(_load(tmp_dir, 'cinclogits', 'LN0', 0)['itr'])
    itr_1 = int(_load(tmp_dir, 'cinclogits', 'LN1', 0)['itr'])
    ...
    for layer in [0, 1]:
        r0 = _load(tmp_dir, 'cinclogits', 'LN0', layer)
        r1 = _load(tmp_dir, 'cinclogits', 'LN1', layer)
        _plot_cinclogits(
            (r0['clogit_icscore'], r0['iclogit_icscore']),
            (r1['clogit_icscore'], r1['iclogit_icscore']),
            plot_dir, attn_layer=layer, tag=tag)

        i0 = _load(tmp_dir, 'intensity', 'LN0', layer)
        i1 = _load(tmp_dir, 'intensity', 'LN1', layer)
        _plot_intensity(
        ...
        for ub in UB_VALUES:
            try:
                u0 = _load(tmp_dir, 'intensity', 'LN0', layer, ub=ub)
                u1 = _load(tmp_dir, 'intensity', 'LN1', layer, ub=ub)
                _plot_intensity(
                    (u0['intensities'], u0['success_rates']),
                    (u1['intensities'], u1['success_rates']),
                    attn_layer=layer, plot_dir=plot_dir, tag=tag, ub_suffix=str(ub))
            except Exception:
                pass

    _assemble_ablation(tmp_dir, plot_dir, tag)
    _assemble_baseline(tmp_dir, plot_dir, tag)
    ...
    for ln in ['LN0', 'LN1']:
        for layer in [0, 1]:
            ...
            if not os.path.exists(f):
                return
            d = np.load(f)
            entry = {'full_seq_acc': float(d['full_seq_acc']), 'per_pos_acc': d['per_pos_acc']}
            if 'cond_acc' in d:
...
def main():
    parser = argparse.ArgumentParser(description="Major plot generation for configs")
    parser.add_argument('config_prefix', type=str,
                        help='Config prefix, e.g. V512_B16_LR1e-02')
    parser.add_argument('--mi', type=int, nargs='+', default=None,
                        help='MI values to process (default: all)')
    parser.add_argument('--dry-run', action='store_true')
    args = parser.parse_args()

    mi_list = args.mi if args.mi else ALL_MI
    print(f"Config prefix: {args.config_prefix}")
    print(f"MI values: {mi_list}")

    pairs = find_pairs(args.config_prefix, mi_list)
    print(f"Found {len(pairs)} complete LN0/LN1 pairs")
    if not pairs:
        print("Nothing to do.")
        return

    all_tasks = make_all_tasks(pairs)
