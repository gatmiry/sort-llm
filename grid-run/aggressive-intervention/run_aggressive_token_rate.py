"""
Aggressive intervention targeting first unsorted token (position 0),
using the SAME intervention mechanism as GPTIntervention:
  SET raw_attn[pos, 0] = raw_attn[pos, correct_token_loc] + intensity
for every sorting-phase position.

Measures per-token success rate (same y-axis as compare_intensity plots).

Config: V256_B16_LR1e-02_MI60000
Layers: 0 and 1
"""
...
CONFIG_BASE = 'V256_B16_LR1e-02_MI60000'
...
def run_aggressive_token_rate(model, config, attn_layer):
    ...
    # Mutable container so the closure sees the current trial's idx
    current_idx = [None]
...
def main():
    ...
    for attn_layer in [0, 1]:
        print(f"\n{'='*60}")
        print(f"  Aggressive First-Token Intervention (Token Rate) — Layer {attn_layer}")
        print(f"  {CONFIG_BASE}")
        print(f"{'='*60}")
        ...
        for ln, model, config in [(0, model_0, cfg_0), (1, model_1, cfg_1)]:
            print(f"  LN{ln}:")
            results = run_aggressive_token_rate(model, config, attn_layer)
            intensities = sorted(results.keys())
            rates = [results[i] for i in intensities]
            np.savez(os.path.join(out_dir, f'aggressive_token_rate_layer{attn_layer}_LN{ln}.npz'),
        ...
        plt.ylim(0, 1.05)
        plt.tight_layout()
        path = os.path.join(out_dir, f'aggressive_token_rate_layer{attn_layer}.png')
