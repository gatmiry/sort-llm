"""
Compare intervention effects between models with and without layer normalization.
Fix intensity at 0.0, vary unsorted_ub_num (number of tokens to intervene on).
"""
import sys
import os
...
num_rounds = 4000
FN = 400
intensity = 0.0  # Fixed intensity
num_values = [0, 1, 2, 3, 4, 5, 6, 7, 8]  # Vary unsorted_ub_num
...
for num in num_values:
    ...
    for _ in range(num_rounds):
        ...
        try:
            intervention_model.intervent_attention(
                ...
                unsorted_lb=5, unsorted_ub=5, unsorted_lb_num=0, unsorted_ub_num=num,
    ...
    results_1[num] = sum(first_fn) / len(first_fn) if len(first_fn) > 0 else 0
    print(f'Num tokens: {num}, Success rate: {results_1[num]:.4f} ({sum(first_fn)}/{len(first_fn)})')
...
for num in num_values:
    ...
    for _ in range(num_rounds):
        ...
        try:
            ...
            continue
    first_fn = attempts[:FN]
    results_2[num] = sum(first_fn) / len(first_fn) if len(first_fn) > 0 else 0
    print(f'Num tokens: {num}, Success rate: {results_2[num]:.4f} ({sum(first_fn)}/{len(first_fn)})')
...
nums = list(results_1.keys())

plt.plot(nums, list(results_1.values()), marker='o', linewidth=1.5, markersize=5, 
         label='without layer norm', color='#1f77b4')
plt.plot(nums, list(results_2.values()), marker='s', linewidth=1.5, markersize=5, 
         label='with layer norm', color='#ff7f0e')

plt.xlabel('Number of Intervened Tokens', fontsize=9)
plt.ylabel('Success Probability', fontsize=9)
plt.title(f'Robustness vs Number of Interventions', fontsize=10)
plt.legend(fontsize=7, loc='lower left')
plt.grid(True, alpha=0.3)
plt.xticks(nums, fontsize=8)
plt.yticks(fontsize=8)
plt.ylim(0, 1.05)
plt.tight_layout()

output_dir = os.path.dirname(__file__)
plt.savefig(os.path.join(output_dir, 'compare_num.png'), dpi=300, bbox_inches='tight')
print(f'\nPlot saved to {os.path.join(output_dir, "compare_num.png")}')
