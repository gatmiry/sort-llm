#!/bin/bash
# Compare LN vs RMSNorm at fixed train length K=50 under Huang eval ranges.
#
# Variants:
#   0) LN_fixed50      (norm_type=layernorm)
#   1) RMS_fixed50     (norm_type=rmsnorm)
#
# Common eval buckets:
#   in-dist:   K=2..50
#   ood-mid:   K=51..100
...
#SBATCH --job-name=huang_norm_cmp
#SBATCH --account=chip
...
#SBATCH --output=logs/huang_norm_cmp-%A_%a.out
#SBATCH --array=0-23
...
GRID_SIZE=$(( ${#LAYER_COUNTS[@]} * ${#MLP_FLAGS[@]} ))   # 4 tasks/seed
TASKS_PER_VARIANT=$(( GRID_SIZE * ${#SEEDS[@]} ))         # 12

TASK_ID=${SLURM_ARRAY_TASK_ID}
VARIANT_INDEX=$(( TASK_ID / TASKS_PER_VARIANT ))          # 0..1
REM=$(( TASK_ID % TASKS_PER_VARIANT ))
SEED_INDEX=$(( REM / GRID_SIZE ))
INNER_ID=$(( REM % GRID_SIZE ))

if (( VARIANT_INDEX < 0 || VARIANT_INDEX > 1 )); then
