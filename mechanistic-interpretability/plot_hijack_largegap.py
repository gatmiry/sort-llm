#!/usr/bin/env python3
"""
Generate hijack_comparison_3way plots for large gaps (15, 20).
Gap 15: controlled batches (barely fits).
Gap 20: random batches filtered by actual gap at each position.
"""
...
from intervene import enable_attention_storage, get_single_batch
...
def precompute_block0(idx):
    ...
    return dict(embed=embed, raw_att=raw_att, v4=v4, causal=causal,
...
def run_controlled(gap, offsets, n_trials):
    """Controlled-gap batches (all gaps = gap)."""
    ...
    for trial in range(n_trials):
        ...
        for p in range(block_size - 1):
            ...
            for off in offsets:
                ...
                results['mlp1'][off][ir].append(mlp1_hijack(pre, idx, qpos, wpos) == wval)
                results['attn2'][off][ir].append(attn_hijack(idx, 1, qpos, wpos) == wval)
                results['attn1'][off][ir].append(attn_hijack(idx, 0, qpos, wpos) == wval)
        if (trial + 1) % 25 == 0:
            el = time.time() - t0
...
def run_random_filtered(target_gap, offsets, n_trials, gap_tol=2):
    """Random batches; use positions where actual gap is within target_gap ± tol."""
    results = {ht: {off: {ir: [] for ir in I_RANGES} for off in offsets}
               for ht in ['mlp1', 'attn1', 'attn2']}
    t0 = time.time()
    positions_found = 0
    for trial in range(n_trials):
        ...
        pre = precompute_block0(idx)
        for p in range(block_size - 1):
            actual_gap = int(sorted_vals[p + 1]) - int(sorted_vals[p])
            if abs(actual_gap - target_gap) > gap_tol:
                continue
            qpos = block_size + 1 + p
            cval = int(sorted_vals[p])
            tval = int(sorted_vals[p + 1])
        ...
        if (trial + 1) % 100 == 0:
            el = time.time() - t0
            eta = el / (trial + 1) * (n_trials - trial - 1)
            print(f"    gap~{target_gap}: {trial+1}/{n_trials}  "
                  f"positions={positions_found}  "
                  f"({el:.0f}s elapsed, ~{eta:.0f}s remaining)", flush=True)
    print(f"    Total positions with gap~{target_gap}: {positions_found}", flush=True)
    return results


def plot_hijack(gap, results, offsets, label_suffix=""):
    ...
    gap_label = f"gap={gap}" if not label_suffix else f"gap\u2248{gap}"
    fig.suptitle(f'Comparison of three hijack interventions ({gap_label}, N512_s1)',
...
def print_counts(results, offsets):
    for ht in ['mlp1', 'attn1', 'attn2']:
        for ir in I_RANGES:
...
if __name__ == '__main__':
    # Gap 15 — controlled batches (max_start = 512 - 15*32 = 32, tight but works)
    gap15_offsets = [30, 45, 60, 75, 90]
    print(f"\n{'='*60}\nGap 15  |  offsets = {gap15_offsets}\n{'='*60}", flush=True)
    r15 = run_controlled(15, gap15_offsets, 250)
    plot_hijack(15, r15, gap15_offsets)
    print_counts(r15, gap15_offsets)

    # Gap 20 — random batches filtered (can't fit 20*32=640 in 512)
    gap20_offsets = [2, 5, 10, 15, 20, 30]
    print(f"\n{'='*60}\nGap ~20  |  offsets = {gap20_offsets}  (random batches, gap_tol=3)\n{'='*60}", flush=True)
    r20 = run_random_filtered(20, gap20_offsets, 4000, gap_tol=3)
    plot_hijack(20, r20, gap20_offsets, label_suffix="approx")
    print_counts(r20, gap20_offsets)
