#!/usr/bin/env python3
"""
Extract horizontal slices from the (x, t) QK heatmaps at specific
y-axis values (x = query L1 target), and plot as 1D curves of score vs t.
"""
...
def main():
    ...
    output_path = ARGS.output or os.path.join(
        os.path.dirname(ARGS.ckpt), "..", "plots", "qk_heatmap_xt_slices.png"
    )
    ...
    for ci, (z, y, label) in enumerate(configs):
        ...
        for si, x_val in enumerate(x_slices):
            if x_val >= vocab_n:
                continue
            scores = hm[x_val, :]
            ax.plot(t_range, scores, color=slice_colors[si], linewidth=1.2,
                    label=f"x={x_val}", alpha=0.85)
            argmax_t = np.argmax(scores)
            ax.axvline(argmax_t, color=slice_colors[si], linewidth=0.5,
                       linestyle=":", alpha=0.4)

        ax.set_xlabel("t (key base token)", fontsize=10)
        ax.set_ylabel("QK score", fontsize=10)
        ...
        for si, x_val in enumerate(x_slices):
            ...
            ax.text(amax, scores[amax] + (ax.get_ylim()[1] - ax.get_ylim()[0]) * 0.01,
                    f"{amax}", fontsize=7, color=slice_colors[si],
                    ha="center", va="bottom", fontweight="bold")

    fig.suptitle(
        f"QK score slices at x = {x_slices}\n"
        f"score(x, t) vs t for fixed x (query L1 target)\n"
