...
class CausalSelfAttention(nn.Module):
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        self.raw_attn = attn.clone().detach().squeeze(0).squeeze(0)
        attn = F.softmax(attn, dim=-1)
        self.stored_attn = attn.clone().detach().squeeze(0).squeeze(0)
        y = attn @ v
...
class Block(nn.Module):
    ...
    def forward(self, x: torch.Tensor, layer_n: int = -1) -> torch.Tensor:
        #if layer_n == 0:
