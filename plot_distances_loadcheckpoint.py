from next_num_statistics_loadcheckpoint import get_statistics, block_size, plots_folder
import numpy as np
import matplotlib
matplotlib.use('Agg')
...
plt.savefig(f'{plots_folder}/distance_perthreshold.png', dpi=150, bbox_inches='tight')
print(f'Plot saved to {plots_folder}/distance_perthreshold.png')
plt.close()
...
plt.legend()
plt.grid(True, alpha=0.3)
plt.savefig(f'{plots_folder}/distance_perlocation.png', dpi=150, bbox_inches='tight')
print(f'Plot saved to {plots_folder}/distance_perlocation.png')
plt.close()
