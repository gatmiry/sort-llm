...
def get_batch():
   def sort_tensor(x):
      ...
      #print('vals are ', vals)
      return vals
   x = torch.stack([sort_tensor(torch.randperm(vocab_size)[:block_size]) for _ in range(batch_size)])
   return x
...
from ddmodel2 import DDGPT, GPTConfig, Perturber
print('im here!')
conf = GPTConfig(block_size=block_size, vocab_size=vocab_size)
mymodel = DDGPT(conf)
num_swaps = 1
perturber = Perturber(conf.n_layers, num_swaps)
device = 'cpu'
...
for itr in range(max_iter):
   ...
   if itr % 2 == 0:
      perturber.swap_count = 1
      pseqs = perturber(x) # compute the perturbed sequences
   else:
      perturber.swap_count = 0
      pseqs = perturber(x)
   logits, loss = mymodel(pseqs)
   ...
   if itr % 10 == 0:
      ...
      x = x.to(device)
      pseqs = perturber(x)
      test_logits, test_loss = mymodel(pseqs)
      mymodel.train()
   ...
   if itr % 5000 == 0:
      ...
      import os
      torch.save(checkpoint, os.path.join(os.getcwd(), f'./saved_ddmodels2/itr_{itr}_checkpoint.pt'))
